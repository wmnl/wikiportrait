...
<div id="content">
	...
	<div class="input-container">
	    ...
	    <input type="text" name="title" id="title" required="required" value="<?php if (!empty($_POST['title'])) echo $_POST['title']; ?>" onblur="comparenames()" />
	</div>

	<div class="input-container">
	    <label for="source"><i aria-hidden="true" class="fa fa-camera fa-lg fa-fw"></i>Heeft u deze foto zelfgemaakt?</label>
	    <div class="radiogroup">
		<input type="radio" name="source_self" value="Yes" id="source_self_yes" required="required" <?php echo (array_key_exists('source_self', $_POST) && $_POST['source_self'] == 'Yes') ? 'checked="checked"' : ''; ?>>Ja, ik heb deze foto zelf gemaakt<br>
		<input type="radio" name="source_self" value="No" id="source_self_no" required="required"  <?php echo (array_key_exists('source_self', $_POST) && $_POST['source_self'] == 'No') ? 'checked="checked"' : ''; ?>>Nee, ik heb deze foto niet zelf gemaakt<br>
		<input type="radio" name="source_self" value="Unknown" id="source_self_unknown" required="required"  <?php echo (array_key_exists('source_self', $_POST) && $_POST['source_self'] == 'Unknown') ? 'checked="checked"' : ''; ?>>Ik weet niet wie de foto gemaakt heeft
	    </div>
	</div>

	<div class="input-container hidden" id="source_container">
	    <label for="source"><i aria-hidden="true" class="fa fa-camera-retro fa-lg fa-fw"></i>Wat is de naam van de fotograaf?</label>
	    <input type="text" name="source" id="source" <?php echo (array_key_exists('source_self', $_POST) && $_POST['source_self'] != 'Yes') ? 'required="required"' : ''; ?> value="<?php if (!empty($_POST['source'])) echo $_POST['source']; ?>" onblur="comparenames()" />
	</div>
	...
	<div class="input-container">
	    <label for="date"><i aria-hidden="true" class="fa fa-calendar fa-lg fa-fw"></i>Wanneer is de foto genomen? <span class="optional">(optioneel)</span></label>
	    <input type="text" name="date" id="date" value="<?php if (!empty($_POST['date'])) echo $_POST['date']; ?>" />
	</div>

	<div class="input-container">
	    <label for="description"><i aria-hidden="true" class="fa fa-comment fa-lg fa-fw"></i>Omschrijving <span class="optional">(optioneel)</span></label>
	...
	<div class="bottom right">
	    ...
	    <button class="green" type='button' onclick='disableButton(this)'><i aria-hidden="true" class="fa fa-cloud-upload fa-lg"></i>Upload foto</button>
	</div>
