	...
	$.each(data,
		function (index, items) {
			...
			if ($.inArray(itemcat, existingCats)==-1) {
			    $html += '<input type="checkbox" name="categories[]" value="Category:' + item.replace('Category:', '') + '" id="categories_' + item.toLowerCase().replace('category:', '').replace(/\s/g, '_') + '"><label for="categories_' + item.toLowerCase().replace('category:', '').replace(/\s/g, '_') + '">' + item.replace('Category:', '') + getTrueFalse(truefalse) + '</label><br>';
			}
		    }.bind(this))
		}.bind(this));
...
<div id="content">
    ...
    <div class="single">
		...
		$owner = DB::queryFirstRow('SELECT owner, archived,ticket,categories FROM images WHERE id = %d', $_GET['id']);
...
?></div></div>
		    ...
		    <button type="button" onclick="getCategories(<?php echo str_replace('"', '\'', json_encode(array($row['title'], $row['source']))) . ',' . str_replace('"', '\'', $owner['categories'] ?? ''); ?>)"> <i class="fa fa-list-alt" aria-hidden="true"></i> Krijg categorieën
		    </button>
